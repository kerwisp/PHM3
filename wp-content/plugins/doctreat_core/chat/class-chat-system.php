...
if (!class_exists('ChatSystem')) {
    ...
    class ChatSystem
    {
        ...
        private function __construct()
        {
			...
			add_action('doctreat_chat_count', array(__CLASS__, 'countUnreadMessages'),11,2);
			add_filter('doctreat_chat_count', array(__CLASS__, 'countUnreadMessages'),11,2);
        ...
        public static function fetchUserThreads($userId = '')
        {
                    ...
                    <div class="dc-formtheme dc-formsearch">
                        <fieldset>
                            <div class="form-group">
                                <input type="text" autocomplete="off" name="fullname" class="form-control dc-filter-users" placeholder="<?php esc_html_e('Search Users', 'doctreat_core'); ?>">
								...
								foreach ( $usersThreadUserList as $userVal ) {
									...
									<div class="dc-ad dc-load-chat <?php echo esc_attr($unreadNotifyClass); ?>" id="load-user-chat-<?php echo intval($chat_user_id); ?>" data-userid="<?php echo intval($chat_user_id); ?>" data-currentid="<?php echo intval($userId); ?>" data-msgid="<?php echo intval($userVal['id']); ?>" data-img="<?php echo esc_url($userAvatar); ?>" data-name="<?php echo esc_attr($userName); ?>" data-url="<?php echo esc_url($userUrl); ?>">
										<figure>
											...
											<?php echo do_action('doctreat_print_user_status',$chat_user_id);?>
										</figure>
                    ...
                    <div class="dc-chatarea load-dc-chat-message">
                        <div class="dc-chatarea dc-chatarea-empty">
                            <figure class="dc-chatemptyimg">
                                <img src="<?php echo esc_url(get_template_directory_uri() . '/images/message-img.png'); ?>" alt="<?php esc_attr_e('No Message Selected', 'doctreat_core'); ?>">
                                <figcaption>
                                    <h3><?php esc_html_e('No message selected to display', 'doctreat_core'); ?></h3>
                                </figcaption>
                            </figure>
                        </div>
                    </div>
            ...
            if(!empty($chat_active_user)){
                ...
                jQuery(window).on('load', function() {
                    ...
                    ";
                wp_add_inline_script( 'doctreat-dashboard', $script, 'before' );
            }
            echo ob_get_clean();
        }
        ...
        public static function fetchUserConversation()
        {
            ...
            if (!empty($_POST) && $receiverID != $senderID ) {
                ...
                if (!empty($usersThreadData)) {
                    ...
                    $json['message'] 				= esc_html__('Chat Messages Found!', 'doctreat_core');
                    wp_send_json($json);
					
                } else {
                    $json['type']       = 'error';
                    $json['message']    = esc_html__('No more messages...', 'doctreat_core');
                    wp_send_json($json);
                }
            } else {
                $json['type']       = 'error';
                $json['message'] = esc_html__('Security check failed!', 'doctreat_core');
                wp_send_json($json);
            }
        ...
        public static function sendUserMessage() 
		{
            ...
            if ( intval( $receiver_id ) === intval( $current_user->ID ) ) {
                $json['type'] = 'error';
                $json['message'] = esc_html__('Something went wrong.', 'doctreat_core');
                wp_send_json($json);
            }

            if (empty($message)) {
                $json['type'] = 'error';
                $json['message'] = esc_html__('Message field is required.', 'doctreat_core');
                wp_send_json($json);
            }
			...
			$receiver_chat_notify = !empty($theme_settings['chat_notify_enable']) ? $theme_settings['chat_notify_enable'] : 'no';

			if (class_exists('Doctreat_Email_helper') && $receiver_chat_notify === 'yes') {
				if (class_exists('DoctreatRecChatNotification')) {
					$email_helper = new DoctreatRecChatNotification();
					$emailData 	  = array();

					$sender_id  	= doctreat_get_linked_profile_id($senderId);
					$receiver_id	= doctreat_get_linked_profile_id($receiverId);
            ...
            if (!empty($msg_id)) {
                ...
                $json['message'] = esc_html__('Message send!', 'doctreat_core');
        ...
        public static function deleteChatMessage()
        {
            ...
            //Response
            $json['type']    = 'success';
            $json['message'] = esc_html__('Message deleted.', 'doctreat_core');
            wp_send_json($json); 
        }
        ...
        public static function getUserInfoData($type = '', $userID = '', $sizes = array()) 
        {
			...
			$postId = doctreat_get_linked_profile_id($userID);

            switch ($type) {
				case "avatar":
					
					if ( apply_filters('doctreat_get_user_type', $userID) === 'doctors' ) {
						$userinfo = apply_filters('doctreat_doctor_avatar_fallback', doctreat_get_doctor_avatar($sizes, $postId), $sizes);
					} else {
						$userinfo = apply_filters('doctreat_doctor_avatar_fallback', doctreat_get_others_avatar($sizes, $postId), $sizes);
					}

					break;
				case "username":
					$userinfo = doctreat_get_username($userID);
					break;
				case "user_register":
					$userinfo = esc_html__('Member Since','doctreat_core').'&nbsp;'.date(get_option('date_format'), strtotime($user_data->user_registered));
					break;
				case "url":
