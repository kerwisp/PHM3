...
if( !class_exists('Payouts_List') ){
	class Payouts_List extends WP_List_Table {
		...
		public function __construct() {
			...
			parent::__construct( [
				'singular' => esc_html__( 'Payout', 'doctreat_core' ), 
				'plural'   => esc_html__( 'Payouts', 'doctreat_core' ),
				'ajax'     => false 
			] );
		...
		public function no_items() {
			esc_html_e( 'No Payouts avaliable.', 'doctreat_core' );
		}
		...
		function get_columns() {
			$columns = array(
				...
				'payment_method'    => esc_html__( 'Payment Method', 'doctreat_core' ),
				'processed_date'    => esc_html__( 'Processing Date', 'doctreat_core' ),
			...
			<body style="font-family: sans-serif;">
				...
				<div style="width:100%; display: inline-block; text-align:center; font-family: sans-serif;">
					<table style="width:96%; margin:80px auto 0;">
						<tr style="text-align:left;">
							<td width="70%">
								<h1 style="font-size: 26px;line-height: 26px;margin: 0 0 10px; font-weight: 500; color: #3d4461;">'.esc_html__('Payout','doctreat_core').'</h1>
								<span style="font-size:16px;line-height: 20px;display: block; color: #3d4461;">'.esc_html__('Payout history starting from','doctreat_core').' '.date('Y-m-d', strtotime($date) ).'</span>
				...
				<footer style="border-top: 1px solid #eee; text-align: center;margin-top: 80px;padding: 20px 0;">
					<span style="display: block;font-size: 16px;color: #3d4461;line-height: 20px;">
						'.esc_html__('This is a computer generated payout slip','doctreat_core').'
					</span>
				</footer>';
			return $html;
		}
		...
		public function renderPdfHtml($results){
			$payrols_list_items	= doctreat_get_payouts_lists();
			$html = '';
			$html .= '<main>
			<table style="width: 95%; margin: 150px auto 0;font-family: sans-serif;">';
				$html .= '<thead>
					<tr style="text-align: left; border-radius:5px 0 0;">
						<th style="width:5%; padding: 15px 20px;background: #f5f5f5; font-size:14px;">'.esc_html__('ID','doctreat_core').'</th>
						<th style="width:15%; padding: 15px 20px;background: #f5f5f5; font-size:14px;">'.esc_html__('Name','doctreat_core').'</th>
						<th style="width:10%; padding: 15px 20px;background: #f5f5f5; font-size:14px;">'.esc_html__('Amount','doctreat_core').'</th>
						<th style="width:10%; padding: 15px 20px;background: #f5f5f5; font-size:14px;word-wrap:break-word;">'.esc_html__('Method','doctreat_core').'</th>
						<th style="width:30%; padding: 15px 20px;background: #f5f5f5; font-size:14px;word-wrap:break-word;">'.esc_html__('Details','doctreat_core').'</th>
						<th style="width:15%; padding: 15px 20px;background: #f5f5f5; font-size:14px;">'.esc_html__('Status','doctreat_core').'</th>
					</tr>
				</thead>
				<tbody>';
			$counter	= 0;
			if( !empty( $results ) ){
				foreach($results as $result ) { 
					...
					if( function_exists('doctreat_get_username') ){
						$user_name	= doctreat_get_username($result['user_id']);
					}
					
					$amounts = '0.0';
					if( function_exists('doctreat_price_format') ){
						$amounts	= doctreat_price_format($result['amount'],'return');
					}
