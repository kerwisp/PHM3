...
if( ! class_exists( 'AMetaboxes' ) ) {
	class AMetaboxes extends MetaboxHelper {
		...
		public function form_process_radio_working( $atts = '' ){
			...
			$am_html .= '<div class="am_option_wraper '.$classes.'">';
				$am_html .=	'<h5>'.esc_html__('Working Time','doctreat_core').'</h5>';
				$am_html .= '<div class="am_option_wraper">';
				$am_html .= '<div class="am_desc">';
				$am_html .= '<label for="am_' . $this->am_sanitize_id( $id ) . '">';
				$am_html .= esc_html__('24/7 Working Time','doctreat_core');
				$am_html .= '</label>';
				$am_html .= '</div>';
				$am_html .= '<div class="am_field">';
				...
				$am_html .= '<div class="am_desc">';
				$am_html .= '<label for="am_' . $this->am_sanitize_id( $id ) . '">';
				$am_html .= esc_html__('Others','doctreat_core');
				$am_html .= '</label>';
				$am_html .= '</div>';
				$am_html .= '<div class="am_field">';
			...
			$am_html .= '<div class="am_option_wraper am_other_time" style="display:' .$display. ';">';
					$am_html .= '<div class="am_desc">';
						$am_html .= '<label for="am_' . $this->am_sanitize_id( $id ) . '">';
						$am_html .= esc_html__('Availability Text','doctreat_core');
						$am_html .= '</label>';
					$am_html .= '</div>';
					$am_html .= '<div class="am_field">';
		...
		public function form_repeater_single( $atts = '' ){
			...
			extract( $atts );
			$db_vals	= array();
			$array_vals	= doctreat_get_post_meta( $post->ID);
						...
						$am_html .= '<div class="repeater-wrap-inner specialities_parents" id="'.$key.'">';
							...
							foreach( $fields as $field ) {
								if( $field['field_type'] === 'specialities' ) {
									$am_html .= '<div class="am_field">';
									$am_html .= '<div class="am_field dropdown-style">';
										$am_html .= apply_filters('doctreat_get_specialities_list','am_specialities['.$key.'][speciality_id]',$key,0);
									$am_html .= '</div>';
									if( !empty( $item )) {
											...
											$am_html .= '<div class="repeater-wrap-inner services-item" id="'.$service_key.'">';
												...
												$am_html .= '<div class="am_field">';
													$am_html .='<div class="am_field dropdown-style related-services">';
														$sp_services	= doctreat_list_service_by_specialities($key);
														...
														if( !empty( $sp_services ) ) {
															...
															$am_html .='</select>';
															$am_html .= '<div class="am_field">';
																$am_html .= '<input type="text" name="am_specialities['.$key.'][services]['.$service_key.'][price]" class="" placeholder="'. esc_html__('Price','doctreat_core').'" value="'.$service['price'].'">';
															$am_html .= '</div>';
															$am_html .= '<div class="am_field">';
																$am_html .= '<textarea placeholder="'. esc_html__('Description','doctreat_core').'" name="am_specialities['.$key.'][services]['.$service_key.'][description]" class="">'.$service['description'].'</textarea>';
															$am_html .= '</div>';
														}
									...
									$am_html .= '<div class="services-wrap">';
										$am_html .= '<div class="system-buttons">';
											$am_html .= '<a href="javascript:;" id="add-service-'.$key.'" data-id="'.$key.'" class="add-repeater-services">'.esc_html__('Add Services','doctreat_core').'</a>';
										$am_html .= '</div>';
									$am_html .= '</div>';
									$am_html .= '</div>';
								} 
							...
							foreach( $fields as $field ) {
								...
								if( $field['field_type'] === 'text' ) {
									...
									$am_html .= '<div class="am_field">';
										...
										$am_html 	.= '</div>';
										$am_html	.= '<div class="system-buttons">';
										$am_html	.= '<span id="upload" class="button system_media_upload_button">'.esc_html__('Upload','doctreat_core').'</span>';
										$am_html 	.= '</div>';
										$am_html 	.= '</div>';
										$am_html 	.= '</div>';
									$am_html .= '</div>';
		...
		public function form_process_upload( $atts = '' ){
			...
			$am_html .= '<div class="am_option_wraper '.$classes.'">';
				...
				$am_html .= '<div class="am_field">';
					...
					$am_html	.= '<div class="system-buttons">';
					$am_html	.= '<span id="upload" class="button system_media_upload_button">'.esc_html__('Upload','doctreat_core').'</span>';
					$am_html	.= '<span style="display:' .$display. ';" id="reset_upload" class="button remove-item" title="Upload">'.esc_html__('Remove','doctreat_core').'</span>';
		...
		public function form_process_gallery( $atts = '' ){
			...
			$am_html .= '<div class="am_option_wraper '.$classes.'">';
				...
				$am_html .= '<div class="am_field">';
					...
					$am_html	.= '<div class="system-buttons">';
					$am_html	.= '<span id="upload" class="button multi_open">'.esc_html__('Add gallery','doctreat_core').'</span>';
					$am_html	.= '<span style="display:' .$display. ';" id="reset_gallery" class="button remove-gallery" title="Upload">'.esc_html__('Remove all','doctreat_core').'</span>';
					$am_html 	.= '</div>';
