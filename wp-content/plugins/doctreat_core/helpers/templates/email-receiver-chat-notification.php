...
if (!class_exists('DoctreatRecChatNotification')) {
    ...
    class DoctreatRecChatNotification extends Doctreat_Email_helper{
		...
		public function send_chat_notification($params = '') {
			...
			$subject_default = esc_html__('A new message received', 'doctreat_core');
			...
			$subject		= !empty( $theme_settings['chat_notify_subject'] ) ? $theme_settings['chat_notify_subject'] : $subject_default;
			$email_content	= !empty( $theme_settings['chat_notify_content'] ) ? $theme_settings['chat_notify_content'] : $contact_default;
	...
	new DoctreatRecChatNotification();
}